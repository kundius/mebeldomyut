...
export default class Projects {
  ...
  init() {
    ...
    forEach(this.thumbs, (el, index) => {
      el.addEventListener('click', e => {
        ...
        SimpleLightbox.open({
          startAt: image,
          items: this.images.map(row => row.sizes['w800h800']),
  ...
  show(index, direction = 'forward') {
    ...
    if (this.active === null) {
      this.main.style.backgroundImage = `url(${this.images[index].sizes['w800h800']})`
      this.main.style.backgroundPosition = `50% 50%`

      this.thumbs[0].style.backgroundImage = `url(${this.images[this.getNextImage(index)].sizes['w800h800']})`
      this.thumbs[0].style.backgroundPosition = `50% 50%`

      this.thumbs[1].style.backgroundImage = `url(${this.images[this.getNextImage(this.getNextImage(index))].sizes['w800h800']})`
      ...
      TweenLite.to(this.wrapper, .3, {
        ...
        onUpdate(self, index) {
          ...
          if (direction === 'forward') {
            self.main.style.backgroundImage = `url(${self.images[self.getPreviousImage(index)].sizes['w800h800']}), url(${self.images[index].sizes['w800h800']})`
            self.main.style.backgroundPosition = `${width*progress*-1}px 50%, ${width-width*progress}px 50%`

            self.thumbs[0].style.backgroundImage = `url(${self.images[index].sizes['w800h800']}), url(${self.images[self.getNextImage(index)].sizes['w800h800']})`
            self.thumbs[0].style.backgroundPosition = `${widthNext*progress*-1}px 50%, ${widthNext-widthNext*progress}px 50%`

            self.thumbs[1].style.backgroundImage = `url(${self.images[self.getNextImage(index)].sizes['w800h800']}), url(${self.images[self.getNextImage(self.getNextImage(index))].sizes['w800h800']})`
            self.thumbs[1].style.backgroundPosition = `${widthAfterNext*progress*-1}px 50%, ${widthAfterNext-widthAfterNext*progress}px 50%`
          } else {
            self.main.style.backgroundImage = `url(${self.images[index].sizes['w800h800']}), url(${self.images[self.getNextImage(index)].sizes['w800h800']})`
            self.main.style.backgroundPosition = `${width*-1 + width*progress}px 50%, ${width*progress}px 50%`

            self.thumbs[0].style.backgroundImage = `url(${self.images[self.getNextImage(index)].sizes['w800h800']}), url(${self.images[self.getNextImage(self.getNextImage(index))].sizes['w800h800']})`
            self.thumbs[0].style.backgroundPosition = `${widthNext*-1 + widthNext*progress}px 50%, ${widthNext*progress}px 50%`

            self.thumbs[1].style.backgroundImage = `url(${self.images[self.getNextImage(self.getNextImage(index))].sizes['w800h800']}), url(${self.images[self.getNextImage(self.getNextImage(self.getNextImage(index)))].sizes['w800h800']})`
